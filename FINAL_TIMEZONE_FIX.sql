-- FINAL TIMEZONE FIX - PRESERVES ORIGINAL TIME WITHOUT CONVERSION
-- This fixes the 18:58 → 22:58 issue by avoiding PostgreSQL's automatic timezone conversions
...
DECLARE
  ...
  new_task_id UUID;
BEGIN
  RAISE NOTICE 'Starting timezone-safe recurring task check...';
  ...
  LOOP
    ...
    -- Calculate next date part based on pattern
    CASE template_record.recurring_pattern
      WHEN 'daily' THEN
        date_part := DATE(task_record.due_date) + INTERVAL '1 day';
      WHEN 'weekly' THEN  
        date_part := DATE(task_record.due_date) + INTERVAL '7 days';
      WHEN 'monthly' THEN
        date_part := DATE(task_record.due_date) + INTERVAL '1 month';
      ELSE
        -- Default to daily
        date_part := DATE(task_record.due_date) + INTERVAL '1 day';
    END CASE;
    
    -- Determine time part (preserve original time or use template time)
    IF template_record.recurring_time IS NOT NULL THEN
      time_part := template_record.recurring_time::TIME;
    ELSE
      -- Use the same time as the original task
      time_part := task_record.due_date::TIME;
    END IF;
    
    -- CRITICAL: Build the timestamp as a string first, then cast
    -- This avoids PostgreSQL's automatic timezone conversions
    next_due_timestamp := (date_part::TEXT || ' ' || time_part::TEXT)::TIMESTAMP;
    ...
    BEGIN
      ...
      RAISE NOTICE 'Generated task: % (%) for % at %', 
                   template_record.title, new_task_id, date_part, time_part;
